...
Page({
    ...
    bindCategoryEvents: function(t) {
        ...
        if(this.data.category_parent_selected ==e){
            this.setData({
        ...
        if(this.data.category_child_selected ==e){
            this.setData({
                category_child_selected: [],
                lastcat:[],
    ...
    bindnav: function(t) {
      if(this.data.ismouse){
        var curTime = t.timeStamp
        console.log(t.timeStamp )
        if (t.timeStamp - this.data.lastTapTime  < 300){
          wx.showModal({
              title:'警告',
              content:'您点击过快可能会出现数据重复，请单击',
              showCancel:false,
              confirmText:'我知道了'
          })
         }
